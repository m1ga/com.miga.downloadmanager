...
#define TAG "com.miga.downloadmanager"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> TiDownloadmanager_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("TiDownloadmanager.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::TiDownloadmanagerBindings::lookupGeneratedInit(
...
static void TiDownloadmanager_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(TiDownloadmanager_getBinding)->GetFunction());
}

static void TiDownloadmanager_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TiDownloadmanagerBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TiDownloadmanagerBinding = {
	"com.miga.downloadmanager",
	TiDownloadmanager_init,
	TiDownloadmanager_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_com_miga_downloadmanager_TiDownloadmanagerBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.miga.downloadmanager", &TiDownloadmanagerBinding);
	titanium::KrollBindings::addExternalLookup(&(::TiDownloadmanagerBindings::lookupGeneratedInit));
}
